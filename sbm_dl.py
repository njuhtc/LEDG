...
class sbm_dataset():
    def __init__(self,args):
        ...
        edges = self.load_edges(args.sbm_args)  # return a list of edges [source, target, weight, time]
        timesteps = u.aggregate_by_time(edges[:,self.ecols.TimeStep], args.sbm_args.aggr_time)   # return a time vector indicating all edge time
        ...
        self.num_classes = edges[:,self.ecols.Weight].unique().size(0) # positive and negative edges

        self.edges = self.edges_to_sp_dict(edges) # dict of edges
