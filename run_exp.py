...
def build_dataset(args):
	if args.data == 'bitcoinotc' or args.data == 'bitcoinalpha':
		...
		return br.Elliptic_Temporal_Dataset(args)
...
def build_gcn(args,tasker):
	...
	if args.model == 'gat':
		return mls.GAT(gcn_args, activation=torch.nn.RReLU()).to(args.device)
...
def build_adpater(args,tasker):
	...
	return mls.Adapter(args,in_features = in_feats).to(args.device)

def build_time_predictor(args,tasker):
	mult = 1
	if 'gru' in args.model or 'lstm' in args.model:
		in_feats = args.gcn_parameters['lstm_l2_feats'] * mult
	elif args.model == 'skipfeatsgcn' or args.model == 'skipfeatsegcn_h':
	...
	return mls.TimePredicter(args,in_features = in_feats).to(args.device)
...
if __name__ == '__main__':
	...
	adpater = build_adpater(args, tasker)
	adpater2 = build_adpater(args, tasker)

	time_predictor = build_time_predictor(args, tasker)
