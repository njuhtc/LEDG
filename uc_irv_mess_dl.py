...
class Uc_Irvine_Message_Dataset():
	def __init__(self,args):
		...
		tar_archive = tarfile.open(tar_file)

		self.edges = self.load_edges(args,tar_archive)
	
	def load_uci_data(self, tar_archive):

		tar_archive = tarfile.open('data/uci.tar.gz')

		f = tar_archive.extractfile('uci/opsahl-ucsocial.edges')
		lines = f.read()#
	...
	def load_edges(self,args,tar_archive):
		data = self.load_uci_data(tar_archive)
